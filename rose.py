#!/usr/bin/python3
# Coded by whiterose
# This is my third project Hehe:)
# Thanks To GARUDA SECURITY
print("""
...
┌────────────────────────────────────────────────────────────┐
│     Warning: Script Ini Di Buat Hanya Untuk Pendidikan     │
├────────────────────────────────────────────────────────────┤
│                         New stuff:                         │
...
├────────────────────────────────────────────────────────────┤
│      https://github.com/whiteroseeee/whiterose-attack      │
└────────────────────────────────────────────────────────────┘''')
...
def ParseUrl(original_url):
	...
	if original_url[:7] == "http://":
		...
		exit()
	#http(s)://www.target.com:1337/xxx ==> www.target.com:1337/xxx
	#print(url) #for debug
	tmp = url.split("/")
	website = tmp[0]#www.target.com:1337/xxx ==> www.target.com:1337
	check = website.split(":")
	...
	target = check[0]
	if len(tmp) > 1:
		path = url.replace(website,"",1)#get the path www.target.com/xxx ==> /xxx
...
nums = 0
def checking(lines,proxy_type,ms,rlock,):#Proxy Check
	global nums
	global proxies
...
def check_socks(ms):#Coded by whiterose
	global nums
	thread_list=[]
